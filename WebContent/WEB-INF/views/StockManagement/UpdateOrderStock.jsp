<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<%@taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<!DOCTYPE html>
<html>
<head>
<title>HealthCare | Details</title>
<meta charset="ISO-8859-1">
<link rel="stylesheet" type="text/css" href="<c:url value="//resources/css/stock.css"/>">
<link rel="stylesheet" type="text/css" href="../shareFiles/index.css"> 
</head>
<body>

<%@ include file="../shareFiles/header.jsp" %>
<div class="row g-0">
<%@ include file="../shareFiles/sideMenu.jsp" %>   
 <div class="col-lg px-5 pt-4 bg-light wow fadeIn rounded"
			data-wow-delay="0.1s">
                        <h1>UPDATE OrderStock</h1>
                      <div class="inputContainer">

 <form action="updateOrderStock">
 <p>Supplier: </p><b>${oDetails.getSupplierName()}</b>
    	<table class="table table-striped table-hover" border="0" cellpadding="0" width="100%" bgcolor="#FFFFFF">
       	<tr>
		<td><b>NO.</b></td>
		<td><b>STOCK</b></td>
        <td><b>QUANTITY</b></td>
		<td></td>
		</tr>
<c:forEach items="${sList}" var="currentStock" varStatus="loop">
<jsp:useBean id="list" class="java.util.ArrayList"/>
<c:set var="myorders" value=""/>
<tr>
	<td><c:out value="${loop.index +1}"/></td>
	<td><input name="name" type="text" value="${currentStock.getName()}" readonly/></td>
	<td><input name="quantity" type="number" value="${currentStock.getQuantity()}"/></td>
	<input name="id" type="hidden" value="${currentStock.getGoodsID()}"/>
	
</tr></c:forEach>
</table>
   <label for="trackingNumber">Tracking Number</label>
    <input type="text" id="trackingNumber" name="trackingNumber" value="${oDetails.getTrackingNumber()}" placeholder="Put the tracking number..." required>
  
  <label for="courier">Choose a courier:</label>
<select id="courier" name="courier">
  <option value="dhl-ecommerce">dhl-ecommerce</option>
  <option value="citylink">Citylink Express</option>
  <option value="skynet">Skynet Express</option>
  <option value="zto">ZTO Express</option>
    <option value="jt">J&T Express</option>
</select><br></br>
    
    <input type="hidden" name="updateindex" value="${oDetails.getId()}"/>
<input type="submit"/>
  </form>

            </div>
            </div>
            </div>
<%@ include file="../shareFiles/footer.jsp" %>
</body>
</html>