...
public class PrescriptionController {
	...
	protected ModelAndView details(HttpServletRequest request) {
	...
	/**
	 * can add throw exception 2. quantity must > 0
	 */
	...
	protected ModelAndView history(HttpServletRequest request) {
		RecomMedicineDAO recomMeddao = new RecomMedicineDAO();
		List<RecomMedicine> sList = recomMeddao
				.findByDoctor(String.valueOf(request.getSession().getAttribute("username")));
	...
	protected ModelAndView filterPatientList(HttpServletRequest request) {
		...
		List<RecomMedicine> sList = recomMeddao.findByPatientAndDoctor(pname,
