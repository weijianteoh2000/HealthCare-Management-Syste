...
public int add(RecomMedicine inst) {
	String sql = "insert into RecomMedicine (patientName,illness,medicine,referenceNo,quantity,dateVisited,doctor) values (?,?,?,?,?,?,?)";
	Object args[] = {inst.getPatientName(),inst.getIllness(),inst.getMedicine(),inst.getReferenceNo(),inst.getQuantity(),inst.getDateVisited(),inst.getDoctor()}; 
...
public int update(RecomMedicine inst, int id) {
	String sql = "Update RecomMedicine SET patientName=?,illness=?,medicine=?,referenceNo=?,quantity=?,dateVisited=? where id=?";
	Object args[] = {inst.getPatientName(),inst.getIllness(),inst.getMedicine(),inst.getReferenceNo(),inst.getQuantity(),inst.getDateVisited(),id}; 
	int rowAffected = jdbct.update(sql, args);
	return rowAffected;
}
